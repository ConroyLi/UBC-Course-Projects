...
timeSteps = 1:length(dragCoefficients(1500:2000,1)); 

figure;
plot(timeSteps, dragCoefficients(1500:2000,1), '-o', 'MarkerSize', 4, 'MarkerFaceColor', 'blue');
...
figure;
plot(timeSteps, liftCoefficients(1500:2000,1), '-x', 'MarkerSize', 4, 'MarkerEdgeColor', 'red');
...
meanDrag = mean(dragCoefficients(1500:2000,1));
disp(['Mean Drag Coefficient: ', num2str(meanDrag)]);


maxAbsLift = max(abs(liftCoefficients(1500:2000,1)));
disp(['Max Absolute Lift Coefficient: ', num2str(maxAbsLift)]);

meanLift =mean(liftCoefficients(1500:2000,1));
disp(['Mean Lift Coefficient: ', num2str(meanLift)]);

rmsdrag = sqrt(mean((dragCoefficients(1500:2000,1)/2-meanDrag).^2));
disp(['RMS of Drag Coefficient: ', num2str(rmsdrag)]);

rmsLift = sqrt(mean((liftCoefficients(1500:2000,1)/2-meanLift).^2));
